...
describe('JID ownership patterns', () => {
  ...
  it('Feishu JID: starts with feishu:', () => {
    const jid = 'feishu:oc_abc123def456';
    expect(jid.startsWith('feishu:')).toBe(true);
  });

  it('Feishu group JID: starts with feishu: with oc_ prefix', () => {
    const jid = 'feishu:oc_test_chat_123';
    expect(jid.startsWith('feishu:')).toBe(true);
    expect(jid.replace('feishu:', '').startsWith('oc_')).toBe(true);
  });
});
...
describe('getAvailableGroups', () => {
  ...
  it('includes Feishu group chat JIDs', () => {
    storeChatMetadata('feishu:oc_abc123', '2024-01-01T00:00:01.000Z', 'Feishu Group', 'feishu', true);
    storeChatMetadata('user@s.whatsapp.net', '2024-01-01T00:00:02.000Z', 'User DM', 'whatsapp', false);

    const groups = getAvailableGroups();
    expect(groups).toHaveLength(1);
    expect(groups[0].jid).toBe('feishu:oc_abc123');
  });

  it('marks registered Feishu chats correctly', () => {
    storeChatMetadata('feishu:oc_reg_001', '2024-01-01T00:00:01.000Z', 'Feishu Registered', 'feishu', true);
    storeChatMetadata('feishu:oc_unreg_002', '2024-01-01T00:00:02.000Z', 'Feishu Unregistered', 'feishu', true);

    _setRegisteredGroups({
      'feishu:oc_reg_001': {
        name: 'Feishu Registered',
        folder: 'feishu-registered',
        trigger: '@Andy',
        added_at: '2024-01-01T00:00:00.000Z',
      },
    });
    ...
    const groups = getAvailableGroups();
    const reg = groups.find((g) => g.jid === 'feishu:oc_reg_001');
    const unreg = groups.find((g) => g.jid === 'feishu:oc_unreg_002');

    expect(reg?.isRegistered).toBe(true);
    expect(unreg?.isRegistered).toBe(false);
  });

  it('mixes WhatsApp, Telegram, and Feishu chats ordered by activity', () => {
    ...
    storeChatMetadata('feishu:oc_abc', '2024-01-01T00:00:05.000Z', 'Feishu', 'feishu', true);
    storeChatMetadata('wa2@g.us', '2024-01-01T00:00:02.000Z', 'WhatsApp 2', 'whatsapp', true);

    const groups = getAvailableGroups();
    expect(groups).toHaveLength(4);
    expect(groups[0].jid).toBe('feishu:oc_abc');
